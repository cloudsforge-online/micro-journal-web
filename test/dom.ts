 ...
 * not function-shaped. On THIS surface the scenario that decides whether the repository has done
 * its job is "with `/v1/pool` failing, the page still says this pool does not pay out" — a
 * statement about a tree of components after their effects have run, which is the one assertion
 * that cannot be made against a pure function because the whole mechanism under test is a
 * provider, an effect and a conditional render. `test/honesty.test.ts` is that scenario, and this
 * file exists so that it can be written at all.
 *
...
/** The form controls a `<label>` may name, per the HTML accname mapping. */
const LABELABLE = new Set(['input', 'select', 'textarea', 'button', 'meter', 'output', 'progress'])

/**
 * The `<label>` naming a form control: `label[for=<id>]` first, then an ancestor `<label>`.
 *
 * This step is NOT optional and its absence was found by a red test rather than by reading. Without
 * it every `<input>` and `<select>` in this repository has an accessible name of the empty string,
 * so `byRole('textbox', /Mining address/)` finds nothing — while a real browser, and every screen
 * reader, announces "Mining address or full stratum username". A harness that disagreed with the
 * browser here would push this repository toward adding `aria-label` to controls that already have
 * a visible `<label>`, which is duplicated markup added to satisfy a test rather than a reader.
 */
function labelText(el: Element): string {
  if (!LABELABLE.has(el.tagName.toLowerCase())) return ''
  const id = el.getAttribute('id')
  if (id) {
    // `getElementById` is not usable here: the association is by `for`, and several controls on a
    // page may share a label's text without sharing its id.
    const forLabel = el.ownerDocument.querySelector(`label[for="${id.replace(/"/g, '\\"')}"]`)
    if (forLabel) return squeeze(forLabel.textContent ?? '')
  }
  for (let node: Element | null = el.parentElement; node; node = node.parentElement) {
    if (node.tagName.toLowerCase() === 'label') return squeeze(node.textContent ?? '')
  }
  return ''
}
 ...
 * `aria-label`, then `aria-labelledby`, then a `<label>`, then the element's own text, then `title`.
 */
...
export async function mount(element: ReactElement, options: MountOptions = {}): Promise<Screen> {
  // This surface's own address on the mainnet estate. It is the default because `cloudsforgeHosts()`
